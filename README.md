...
希望电脑在固定时刻或固定间隔自动跑**与上面相同的增量同步**，请用本机「任务计划程序」（Windows）或 `cron` / `systemd`（Linux）调用：

```bash
uv run onelap2strava sync --incremental
```

「每隔多久、每天几点」在**系统调度器**里设置，不是 CLI 参数。配置步骤、工作目录与退出码说明见 [contexts/phase4-scheduled-sync.md](contexts/phase4-scheduled-sync.md)。
...
**Q: 想用定时任务自动跑增量同步怎么配？**
A: 用系统自带的计划任务或 `cron` 定期执行与手动相同的命令：`uv run onelap2strava sync --incremental`。间隔、每天几点在调度器里配置，本工具不提供 `--interval` 一类参数。完整示例（含 Windows「起始于」、`PATH`、退出码）见 [contexts/phase4-scheduled-sync.md](contexts/phase4-scheduled-sync.md)。
