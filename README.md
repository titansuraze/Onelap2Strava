...
`sync` 的处理链：拉列表 → 下载 Fit 到 `data/cache/` → 本地 SQLite 同步日志查重（开始时间 ±10min + 时长差 <5% + 起点 <500m 三元组，命中即跳过）→ 修正坐标 → Strava 按开始时间 ±10min 查重兜底 → 上传修正后的 Fit → 轮询到 Strava 处理完成。上传过程中遇到网络瞬时错误自动重试（指数退避 1s/2s/4s，最多 3 次）。

每次同步的结果（成功 / 本地去重 / Strava 去重 / 失败）都会写入 `data/.sync.db`，可用 `uv run onelap2strava sync-log` 查看。第一次跑时如果 `data/cache/` 里已经有历史 Fit，会自动扫描并 backfill 到日志里，这样模糊去重从第一次就开始生效。

**`--incremental` 如何判定"新"？** 按顽鹿 activity id。同步日志里标记为 `ok` / `duplicate` 的行被视作已处理，下次跑时顽鹿列表里的这些 id 会被过滤掉。`failed` 状态不算已处理，下次会再试一次（重试友好）。
...
回放每次 `sync` 的结果：上传成功 / 本地模糊去重 / Strava 去重 / 失败，用来调试"为什么这条没传"或"上次是什么时候同步的"。日志存在 `data/.sync.db`，删掉就重置状态（下次 `sync` 会从 `data/cache/` 重新 backfill）。
