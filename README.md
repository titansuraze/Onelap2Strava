...
「每隔多久、每天几点」由**系统调度**表达（不是 `sync` 的子参数）。仓库根目录提供一键注册脚本（编辑脚本顶部的 `SYNC_MODE` / 间隔 / 时刻后执行一次即可）：

| 系统 | 一次性执行 | 注册定时任务 |
| --- | --- | --- |
| Windows | `run-incremental-sync.cmd` | `install-scheduled-sync-windows.cmd`（可选参数 `uninstall` 删除任务） |
| Linux / macOS | `chmod +x run-incremental-sync.sh` 后 `./run-incremental-sync.sh` | `./install-scheduled-sync-unix.sh`（`--remove` 移除 crontab 条目） |
...
**Q: 想用定时任务自动跑增量同步怎么配？**
A: 最简单：用仓库根目录的安装脚本——Windows 运行 `install-scheduled-sync-windows.cmd`（先改脚本里的 `SYNC_MODE` / `HOURLY_INTERVAL` 或 `DAILY_TIME`），Linux/macOS 先 `chmod +x install-scheduled-sync-unix.sh` 再执行（同样可改脚本内变量或用环境变量覆盖）。也可手动用计划任务 / `cron` 调用 `uv run onelap2strava sync --incremental`；本工具不提供 `--interval` 参数。更多示例与退出码见 [contexts/phase4-scheduled-sync.md](contexts/phase4-scheduled-sync.md)。
...
├── run-incremental-sync.cmd           # Windows：单次增量同步（供计划任务调用）
├── install-scheduled-sync-windows.cmd # Windows：注册/卸载计划任务（脚本内改间隔或每天时刻）
├── run-incremental-sync.sh            # Linux/macOS：单次增量同步
├── install-scheduled-sync-unix.sh     # Linux/macOS：写入/移除 crontab
├── README.md               # 本文件
