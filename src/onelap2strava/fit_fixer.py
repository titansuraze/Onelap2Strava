...
def _extract_metadata(fit: FitFile) -> tuple[datetime | None, str | None]:
    """Pull out activity start time (UTC) and sport string for logging/dedup."""
    ...
    for r in fit.records:
        ...
        if cls == "SessionMessage":
            ...
            if ts_ms is not None:
                start_time_utc = datetime.fromtimestamp(ts_ms / 1000, tz=timezone.utc)
    # Fall back to the first record's timestamp if we still have nothing.
...
def fix_fit(input_path: Path | str, output_path: Path | str | None = None) -> FixResult:
    ...
    start_time_utc, sport = _extract_metadata(fit)
